...
inline score_t scorePos(const array<array<int, 4>, 3> cnt, const array<score_t, 4> pi){
	//   AAGG*2*Y^2 + AAYY*2*G*R - YYAG*A*R - AAGY*2*R*Y
	// + AAGG*2*Y^2 + GGYY*2*A*R - YYAG*G*R - GGAY*2*R*Y
	//---------------------------------------------------
	// + AAGG*4*Y^2              - YYAG*R^2
	...
	if (R > Y){
		...
		return (R - Y) * R / Y * (aagg + aayy + ggyy - yyag - aagy - ggay);
	}
	else{
		...
		return (Y - R) * Y / R * (cctt + ccrr + ttrr - rrct - cctr - ttcr); 
	}
}
