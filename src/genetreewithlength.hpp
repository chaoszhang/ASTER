...
struct Tripartition{
	struct Partition{
		...
		score_t normal(Node& w){
			...
			w.x2b = 0; //(u.x2b + v.x2b + u.x * v.x) * w.weight * w.weight;
			w.y2b = 0; //(u.y2b + v.y2b + u.y * v.y) * w.weight * w.weight;
			w.z2b = 0; //(u.z2b + v.z2b + u.z * v.z) * w.weight * w.weight;
			w.xyb = 0; //(u.xyb + v.xyb + u.x * v.y + u.y * v.x) * w.weight * w.weight;
			w.xzb = 0; //(u.xzb + v.xzb + u.x * v.z + u.z * v.x) * w.weight * w.weight;
			w.yzb = 0; //(u.yzb + v.yzb + u.y * v.z + u.z * v.y) * w.weight * w.weight;
